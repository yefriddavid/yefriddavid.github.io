// One-time backfill: set platform = 'binance_arg' on existing Finance_Crypto_Withdrawals
...
//
// Usage:
//   node scripts/backfill-crypto-withdrawal-platform.mjs             (dry run, production data)
//   node scripts/backfill-crypto-withdrawal-platform.mjs --apply     (write changes, production)
//   node scripts/backfill-crypto-withdrawal-platform.mjs --test            (dry run, test project)
//   node scripts/backfill-crypto-withdrawal-platform.mjs --test --apply    (write changes, test project)
...
const COLLECTION = 'Finance_Crypto_Withdrawals'
const BATCH_SIZE = 499
const PLATFORM_VALUE = 'binance_arg'
...
console.log(`\n🔧  backfill-crypto-withdrawal-platform`)
...
console.log(`Retiros totales         : ${snap.size}`)
console.log(`Con plataforma ya       : ${snap.size - toUpdate.length}`)
console.log(`A completar             : ${toUpdate.length}\n`)

toUpdate.forEach((d) => {
  const { applyTime, coin, amount } = d.data()
  console.log(`  ${d.id}  ${applyTime}  ${coin} x${amount}  → platform "${PLATFORM_VALUE}"`)
})
...
console.log(`\n✅  Listo. ${toUpdate.length} retiros actualizados con platform "${PLATFORM_VALUE}".`)
process.exit(0)
