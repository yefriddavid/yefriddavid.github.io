...
export const DetailRow = ({ label, value, mono }) =>
  value != null && value !== '' ? (
    <div style={{ display: 'flex', gap: 8, padding: '5px 0', borderBottom: '1px solid var(--cui-border-color, #e8e8e8)' }}>
      <span style={{ minWidth: 150, fontSize: 12, color: 'var(--cui-secondary-color, #6c757d)', fontWeight: 500 }}>{label}</span>
...
export const DetailSection = ({ title, children }) => (
  <div>
    <p style={{
      ...
      letterSpacing: 1, color: 'var(--cui-secondary-color, #6c757d)',
      margin: '0 0 6px',
    }}>
      {title}
    </p>
