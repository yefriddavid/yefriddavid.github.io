...
import { cilAccountLogout, cilMenu, cilApplications, cilCog } from '@coreui/icons'
import { AppSidebarNav } from '../layout/AppSidebarNav'
import { signOut } from '../../services/firebase/auth'
import { sygnet } from 'src/assets/brand/sygnet'
import getMyToolsNav from '../../_nav.mytools'

const MyToolsSidebar = () => {
  const dispatch = useDispatch()
  const { t } = useTranslation()
  const navigate = useNavigate()
  const unfoldable = useSelector((state) => state.ui.sidebarUnfoldable)
  const sidebarShow = useSelector((state) => state.ui.sidebarShow)
  const navigation = getMyToolsNav()
    ...
    >
      <CSidebarHeader className="border-bottom">
        <div className="d-flex align-items-center gap-2">
          <CIcon icon={cilCog} height={20} className="text-warning" />
          <h3 className="mb-0" style={{ fontSize: '1.2rem' }}>My Tools</h3>
        </div>
        <CSidebarBrand to="/mytools/home">
...
export default React.memo(MyToolsSidebar)
