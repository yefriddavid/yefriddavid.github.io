...
const COLLECTIONS = [
  ...
  { name: 'CashFlow_account_status_period_notes', label: 'Notas estado de cuentas', module: 'CashFlow' },
  // Vauchers (dynamic by year)
  { name: `paymentVauchers-${currentYear}`, label: `Vauchers ${currentYear}`, module: 'Vauchers' },
  { name: `paymentVauchers-${currentYear - 1}`, label: `Vauchers ${currentYear - 1}`, module: 'Vauchers' },
  // Security
...
export async function fetchCollectionCounts() {
  ...
  return results.map((r, i) =>
    r.status === 'fulfilled'
      ? r.value
      : { ...COLLECTIONS[i], count: null, error: true },
  )
}
