...
function PivotedTable({ computed, allTargetKeys, amountByTarget, eggPrice }) {
  ...
  return (
    <div style={{ overflowX: 'auto' }}>
      ...
      <table
        style={{ width: '100%', borderCollapse: 'collapse', minWidth: allKeys.length * colW + 180 }}
      >
        <thead>
        ...
        <tbody>
          {computed.map((d, i) => {
            ...
            const stickyBg = isMatch ? 'linear-gradient(90deg, #fff3cd 0%, #fffce8 100%)' : baseBg
  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        <label
          htmlFor="summary-pivot"
          style={{ cursor: 'pointer', fontSize: 13, color: '#495057', userSelect: 'none' }}
        >
      ...
      {pivoted ? (
        <PivotedTable
          computed={computed}
          allTargetKeys={allTargetKeys}
          amountByTarget={amountByTarget}
          eggPrice={eggPrice}
        />
      ) : (
        <div style={{ overflowX: 'auto' }}>
          <table
            style={{
              ...
              minWidth: distributions.length * colW + 180,
            }}
          >
            <thead>
              <tr style={{ background: '#f8f9fa' }}>
            ...
            <tbody>
              <tr style={{ background: '#f0fff4' }}>
                ...
                {computed.map((d) => (
                  <td
                    key={d.id}
                    style={{ ...cell, textAlign: 'right', fontWeight: 700, color: '#2f9e44' }}
                  >
                    {fmt(d.salary)}
                  </td>
                ))}
              ...
              <tr style={{ background: '#faf5ff' }}>
                ...
                {computed.map((d) => (
                  <td
                    key={d.id}
                    style={{ ...cell, textAlign: 'right', fontWeight: 700, color: '#7c3aed' }}
                  >
                    {d.invert > 0 ? (
                      `−${fmt(d.invert)}`
                    ) : (
              ...
              >
                <td style={{ ...cell, ...sticky, fontWeight: 700, background: '#f8f9fa' }}>
                  Base a distribuir
                </td>
                {computed.map((d) => (
                  <td
