...
function GridChart({
  ...
  const levelH = compact ? 34 : 44
  const W = compact ? 240 : 360
  const H = (gc + 1) * levelH + 60
  const axisX = compact ? 95 : 115
  const lineEndX = W - (compact ? 60 : 110)
  ...
  return (
    ...
    >
      ...
      {/* Arrow tip */}
      <polygon points={`${axisX},${12} ${axisX - 3.5},${20} ${axisX + 3.5},${20}`} fill="#1e293b" />
      ...
      {/* Center price line */}
      <line x1={axisX} y1={centerY} x2={lineEndX} y2={centerY} stroke="#fbbf24" strokeWidth={1.5} />
      ...
      {/* Circle marker on axis at center price */}
...
// ── Trade card ─────────────────────────────────────────────────────────────────
function TradeCard({ trade, onEdit, onDelete, expanded, onToggle }) {
