...
const fmtNum = (n, decimals = 6) =>
  n != null && n !== '' ? new Intl.NumberFormat('es-CO', { maximumFractionDigits: decimals }).format(n) : '—'
...
const EMPTY = {
  name: '', quantity: '', unitPrice: '', type: 'financial',
  liquid: false, projection: false, horizon: '', monthlyGain: '', archived: false, notes: '',
}
    ...
    <div style={{
      ...
      {(asset.quantity || asset.unitPrice) && (
        <div style={{
          ...
          fontSize: 12, color: '#6c757d', marginBottom: 8,
        }}>
          <span>{fmtNum(asset.quantity)} unidades</span>
          {asset.unitPrice ? <><span style={{ color: '#dee2e6' }}>·</span><span>precio {fmt(asset.unitPrice)}</span></> : null}
        </div>
      )}
      ...
      <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
        ...
        <button
          ...
          style={{
            ...
            background: '#eef4ff', fontSize: 13, fontWeight: 600,
            color: syncing ? '#adb5bd' : '#1e3a5f', cursor: syncing ? 'not-allowed' : 'pointer',
          }}
...
function AssetSheet({ initial, saving, onSave, onClose }) {
  ...
  const inputStyle = {
    width: '100%', border: 'none', borderBottom: '2px solid #dee2e6',
    outline: 'none', padding: '4px 0 8px', background: 'transparent',
  ...
  return (
    <div onClick={onClose} style={{
      ...
      zIndex: 1050, display: 'flex', alignItems: 'flex-end', justifyContent: 'center',
    }}>
      <div onClick={(e) => e.stopPropagation()} style={{
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {TYPES.map((t) => (
              <button key={t} style={toggleStyle(form.type === t, TYPE_COLOR[t], TYPE_BG[t])}
        ...
        {valueCOP > 0 && (
          <div style={{
            background: '#eef4ff', border: '1px solid #c5d8ff', borderRadius: 10,
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            ...
            {HORIZONS.map((h) => (
              <button key={h} style={toggleStyle(form.horizon === h, HORIZON_COLOR[h], HORIZON_BG[h])}
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <textarea
            ...
            rows={2} value={form.notes} onChange={set('notes')} placeholder="Observaciones…"
          />
        </div>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={onClose} style={{
            padding: '13px', borderRadius: 12, border: '1px solid #dee2e6',
            background: '#fff', fontSize: 14, fontWeight: 600, color: '#6c757d', cursor: 'pointer',
          }}>Cancelar</button>
          <button
            onClick={handleSave}
...
// ── Summary card ──────────────────────────────────────────────────────────────
function SummaryCard({ label, value, sub, color = '#1a1a2e', bg = '#f8f9fa', border = '#e9ecef', wide = false }) {
  return (
    <div style={{
      ...
      <div style={{ fontSize: 10, fontWeight: 700, color: '#adb5bd', letterSpacing: '0.05em', marginBottom: 4 }}>{label}</div>
...
export default function Assets() {
  ...
  useEffect(() => { dispatch(actions.loadRequest()) }, [dispatch])
  ...
  const totals = useMemo(() => {
    let total = 0, financial = 0, fixed = 0, liquid = 0, monthly = 0
    assets.forEach((a) => {
  ...
  const handleDelete = (asset) => {
    if (window.confirm(`¿Eliminar "${asset.name}"?`)) dispatch(actions.deleteRequest({ id: asset.id }))
  }
  ...
  return (
    <div style={{ maxWidth: 680, margin: '0 auto', padding: '0 12px 60px', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 0 14px' }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={handleImport} disabled={importing} title="Importar desde Firebase" style={{
            width: 44, height: 44, borderRadius: '50%', border: '1px solid #dee2e6',
            background: '#fff', color: importing ? '#adb5bd' : '#1e3a5f', fontSize: 18,
          ...
          <button onClick={toggleView} style={{
            width: 44, height: 44, borderRadius: '50%', border: '2px solid #dee2e6',
            background: '#fff', color: '#6c757d', fontSize: 16, cursor: 'pointer',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
          ...
          <button onClick={() => setSheet('new')} style={{
            ...
            background: '#1e3a5f', color: '#fff', fontSize: 22, cursor: 'pointer',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
          }}>+</button>
        </div>
      </div>
      ...
      {!loading && assets.length > 0 && (
        <>
          ...
          <div style={{
            ...
            <div style={{ fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.6)', letterSpacing: '0.08em', marginBottom: 6 }}>
              PORTFOLIO TOTAL
            </div>
      ...
      <div style={{ position: 'relative', marginBottom: 10 }}>
        ...
        <input
          ...
          style={{
            width: '100%', padding: '9px 10px 9px 34px', borderRadius: 10,
            border: '1px solid #dee2e6', background: '#fff', fontSize: 14, outline: 'none',
          }}
        />
        {search && (
          <button onClick={() => setSearch('')} style={{
            position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)',
            background: 'none', border: 'none', color: '#adb5bd', cursor: 'pointer', fontSize: 16,
          }}>×</button>
        )}
      </div>
      ...
      {unsyncedCount > 0 && (
        <button
          ...
          style={{
            ...
            fontSize: 14, fontWeight: 700, cursor: syncingAll ? 'not-allowed' : 'pointer',
      ...
      {loading ? (
        <div style={{ display: 'flex', justifyContent: 'center', padding: 48 }}>
          <div style={{ width: 32, height: 32, border: '3px solid #dee2e6', borderTopColor: '#1e3a5f', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
        </div>
      ) : filtered.length === 0 ? (
        ...
        >
          ...
          <Column dataField="horizon" caption="Horizonte" width={90}
            cellRender={({ value }) => value ? (
              <span style={{ color: HORIZON_COLOR[value], fontWeight: 700, fontSize: 12 }}>{value}</span>
            ) : null}
          />
          <Column dataField="monthlyGain" caption="Ganancia/mes" dataType="number" width={130}
            format={{ type: 'currency', currency: 'COP', precision: 0 }} />
          <Column dataField="projection" caption="Proyección" dataType="boolean" width={90} />
          <Column caption="" width={80}
            cellRender={({ data }) => (
              <div style={{ display: 'flex', gap: 4 }}>
                <button onClick={() => setSheet(data)} style={{ border: 'none', background: 'none', cursor: 'pointer', fontSize: 14, color: '#1e3a5f' }}>✏️</button>
                <button onClick={() => handleDelete(data)} style={{ border: 'none', background: 'none', cursor: 'pointer', fontSize: 14, color: '#e03131' }}>🗑</button>
              </div>
            )}
          />
          <Summary>
        ...
        filtered.map((a) => (
          <AssetCard key={a.id} asset={a} onEdit={setSheet} onDelete={handleDelete} onSync={handleSync} syncing={syncing} />
        ))
      )}
