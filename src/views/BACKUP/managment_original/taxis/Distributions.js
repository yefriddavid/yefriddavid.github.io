...
const Distributions = () => {
  ...
  const handleGenerate = () => {
    if (!window.confirm(t('taxis.distributions.confirmGenerate', { month: months[period.month - 1], year: period.year }))) return
    const payments = {}
    partners.forEach((p) => {
      payments[p.id] = {
        partnerName: p.name,
  ...
  const handleDeleteDistribution = () => {
    ...
    if (!window.confirm(t('taxis.distributions.confirmDelete'))) return
  ...
  return (
    <>
      ...
      <CModal visible={!!payModal} onClose={() => setPayModal(null)}>
        <CModalHeader>
          <CModalTitle>{t('taxis.distributions.pay.title', { partnerName: payModal?.partnerName })}</CModalTitle>
        </CModalHeader>
        <CModalBody>
          <CForm>
            <CRow className="g-3">
              <CCol xs={12}>
                <div style={{ fontSize: 13, color: 'var(--cui-secondary-color)', marginBottom: 4 }}>
                  {t('taxis.distributions.pay.calculatedAmount')} <strong>{fmt(payModal?.calculatedAmount)}</strong>
                </div>
              </CCol>
              <CCol sm={6}>
                <CFormLabel style={{ fontSize: 12 }}>{t('taxis.distributions.pay.paidAmount')}</CFormLabel>
                <CFormInput
                  type="number"
              ...
              <CCol sm={6}>
                <CFormLabel style={{ fontSize: 12 }}>{t('taxis.distributions.pay.paymentDate')}</CFormLabel>
                <CFormInput
                  type="date"
        ...
        <CModalFooter>
          <CButton color="secondary" variant="outline" onClick={() => setPayModal(null)}>{t('common.cancel')}</CButton>
          <CButton color="primary" onClick={handleMarkPaid} disabled={loadingDist}>
            {loadingDist ? <CSpinner size="sm" /> : t('taxis.distributions.pay.confirm')}
          </CButton>
        </CModalFooter>
      </CModal>
      ...
      <CRow className="mb-3 d-none d-sm-flex">
        <CCol sm={3}>
          <CCard className="text-center">
            <CCardBody>
              <div style={{ fontSize: 12, color: 'var(--cui-secondary-color)', marginBottom: 4 }}>{t('taxis.distributions.summary.totalSettlements')}</div>
        ...
        <CCol sm={3}>
          <CCard className="text-center">
            <CCardBody>
              <div style={{ fontSize: 12, color: 'var(--cui-secondary-color)', marginBottom: 4 }}>{t('taxis.distributions.summary.totalExpenses')}</div>
        ...
        <CCol sm={3}>
          <CCard className="text-center">
            <CCardBody>
              <div style={{ fontSize: 12, color: 'var(--cui-secondary-color)', marginBottom: 4 }}>{t('taxis.distributions.summary.netAvailable')}</div>
          ...
          <CCard className="text-center">
            <CCardBody>
              <div style={{ fontSize: 12, color: 'var(--cui-secondary-color)', marginBottom: 4 }}>
                {distribution
                  ? t('taxis.distributions.summary.pendingPayment')
                  : t('taxis.distributions.summary.assignedPercentage', { percentage: totalPercentage })}
              </div>
      ...
      <CCard>
        <CCardHeader className="d-flex align-items-center justify-content-between flex-wrap gap-2">
          <div className="d-flex align-items-center gap-2">
            <strong>{t('taxis.distributions.title')}</strong>
            {distribution && <CBadge color="secondary">{paymentRows.length} {t('taxis.distributions.partners')}</CBadge>}
            {loadingDist && <CSpinner size="sm" />}
          </div>

          {/* Period selector */}
          <div className="d-flex align-items-center gap-2">
            <span style={{ fontSize: 12, color: 'var(--cui-secondary-color)', whiteSpace: 'nowrap' }}>{t('taxis.distributions.period')}</span>
            <select
          ...
          <div className="d-flex align-items-center gap-2">
            ...
            {distribution && (
              <CButton size="sm" color="danger" variant="outline" onClick={handleDeleteDistribution} title={t('taxis.distributions.deleteDistribution')}>
            ...
            {!distribution && partners.length > 0 && (
              <CButton size="sm" color="primary" onClick={handleGenerate} disabled={loadingDist}>
                ...
                {' '}{t('taxis.distributions.generate')}
              </CButton>
            )}
          </div>
        </CCardHeader>
        ...
        <CCardBody style={{ padding: 0 }}>
          {loading ? (
            ...
            <div className="text-center py-5" style={{ color: 'var(--cui-secondary-color)', fontSize: 14 }}>
              {t('taxis.distributions.noPartners')}
            </div>
          ) : (
            <>
              {!distribution && (
                <div style={{ padding: '12px 16px', background: 'var(--cui-warning-bg-subtle, #fff3cd)', borderBottom: '1px solid var(--cui-border-color)', fontSize: 13 }}>
                  {t('taxis.distributions.previewHintStart')} <strong>{t('taxis.distributions.generate')}</strong> {t('taxis.distributions.previewHintEnd')}
                </div>
              )}
              <DataGrid
                id="paymentsGrid"
                ...
                noDataText={t('taxis.distributions.noData')}
                onRowPrepared={(e) => {
              ...
              >
                ...
                <Column
                  ...
                  cellRender={({ data }) =>
                    data.paid ? (
                      <CBadge color="success">{t('payments.status.paid')}</CBadge>
                    ) : (
                      <CBadge color="warning">{t('payments.status.pending')}</CBadge>
                    )
                  }
                />
                {distribution && (
                  <Column
                    ...
                    cellRender={({ data }) =>
                      !data.paid ? (
                        ...
                        >
                          {t('taxis.distributions.markPaid')}
                        </CButton>
                      ) : null
                    }
                  />
              ...
              {distribution && paymentRows.some((r) => r.paid) && (
                <div style={{ padding: '8px 24px 16px', textAlign: 'right', fontSize: 13 }}>
                  <span style={{ color: 'var(--cui-secondary-color)', marginRight: 12 }}>
                    {t('taxis.distributions.totalPaid')}
                  </span>
