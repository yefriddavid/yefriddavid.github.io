...
export const getColombianHolidays = (year) => {
  ...
  const easterOffset = (n) => {
    const d = new Date(easter)
    d.setDate(d.getDate() + n)
    return d
  }
  add(easterOffset(-3))
  add(easterOffset(-2)) // Jueves y Viernes Santo
...
export const buildAuditDay = (
  ...
  const driverOnDay = (pl) =>
    periodDrivers.find((dr) => {
  ...
  const missingVehicles = expectedVehicles.filter(
    (pl) => !settledVehicles.has(pl) && !picoPlacaSet.has(pl),
  )
  ...
  const status = isFuture
    ? 'future'
    : dayRecords.length === 0 && !hasPicoPlaca
      ? 'none'
      : hasIssue
        ? 'partial'
        : 'full'
  ...
  return {
    ...
    dayRecords,
    settled: [...settled],
    settledVehicles: [...settledVehicles],
    missing,
    missingVehicles,
    underpaidVehicles,
    picoPlacaVehicles,
