...
describe('auditNoteId', () => {
  ...
  it('handles multiple spaces', () => {
    expect(auditNoteId('2024-01-01', 'María Del Carmen López')).toBe(
      '2024-01-01__María_Del_Carmen_López',
    )
  })
...
describe('buildAuditDay', () => {
  ...
  describe('driverOnDay (startDate / endDate)', () => {
    ...
    it('vehicle IS expected within startDate–endDate window', () => {
      const boundedDriver = makeDriver({
        ...driver,
  ...
  describe('isFuture when auditToday is null', () => {
    ...
    it('future year → future', () => {
      const day = buildAuditDay(1, {
        ...
        monthStr: '2026-01',
        now: new Date(2024, 2, 20),
        holidays: getColombianHolidays(2026),
      })
      expect(day.isFuture).toBe(true)
    })

    it('isToday is always false when auditToday is null', () => {
      const day = buildAuditDay(20, {
        ...nullTodayBase,
        year: 2024,
        month: 3,
        monthStr: '2024-03',
  ...
  describe('driver without defaultVehicle', () => {
    const noPlateDriver = makeDriver({
      name: 'Sin Placa',
      defaultVehicle: null,
      defaultAmount: 40000,
    })
    ...
    it('driver without plate not in missing when their name is in settled', () => {
      const records = [
        makeSettlement({ driver: 'Sin Placa', plate: null, amount: 40000, date: '2024-03-05' }),
      ]
  ...
  describe('pico y placa (vehicleRestrictions)', () => {
    ...
    const restrictions = new Map([['ABC123', new Set([5, 10])]])
  ...
  describe('records with falsy driver or plate', () => {
    it('records without driver field are excluded from settled names', () => {
      const records = [
        makeSettlement({ driver: null, plate: 'ABC123', amount: 50000, date: '2024-03-05' }),
      ]
      const day = buildAuditDay(5, { ...baseParams, periodRecords: records })
    ...
    it('records without plate field are excluded from settledVehicles', () => {
      const records = [
        makeSettlement({ driver: 'Juan Perez', plate: null, amount: 50000, date: '2024-03-05' }),
      ]
      const day = buildAuditDay(5, { ...baseParams, periodRecords: records })
      expect(day.settled).toContain('Juan Perez')
  ...
  describe('dow (day of week) value', () => {
    it('Monday = 1', () =>
      expect(buildAuditDay(4, { ...baseParams, periodRecords: [] }).dow).toBe(1)) // Mar 4 2024 = Mon
    it('Tuesday = 2', () =>
      expect(buildAuditDay(5, { ...baseParams, periodRecords: [] }).dow).toBe(2)) // Mar 5 2024 = Tue
    it('Wednesday = 3', () =>
      expect(buildAuditDay(6, { ...baseParams, periodRecords: [] }).dow).toBe(3))
    it('Thursday = 4', () =>
      expect(buildAuditDay(7, { ...baseParams, periodRecords: [] }).dow).toBe(4))
    it('Friday = 5', () =>
      expect(buildAuditDay(8, { ...baseParams, periodRecords: [] }).dow).toBe(5))
    it('Saturday = 6', () =>
      expect(buildAuditDay(9, { ...baseParams, periodRecords: [] }).dow).toBe(6))
    it('Sunday = 0', () =>
      expect(buildAuditDay(10, { ...baseParams, periodRecords: [] }).dow).toBe(0)) // Mar 10 2024 = Sun
  })
})
