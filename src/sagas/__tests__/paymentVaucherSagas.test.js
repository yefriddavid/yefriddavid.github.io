...
describe('paymentVaucherSagas', () => {
  describe('createPaymentVaucher', () => {
    it('beginRequestCreate → CreatePaymentVaucher(payload) → successRequestCreate(response.data)', () => {
      ...
      expect(gen.next().value).toEqual(call(apiPaymentVaucherServices.CreatePaymentVaucher, payload))
      expect(gen.next(response).value).toEqual(put(paymentVaucherActions.successRequestCreate(response.data)))
