...
const contractModuleNoteSlice = createSlice({
  name: 'contratoModuleNote',
  initialState: { notes: [], fetching: false, saving: false, isError: false },
  reducers: {},
  extraReducers: (builder) => {
    builder
      .addCase(actions.fetchRequest, (state) => { state.fetching = true; state.isError = false })
      .addCase(actions.successRequestFetch, (state, { payload }) => { state.notes = payload; state.fetching = false })
      ...
      .addCase(actions.successRequestCreate, (state, { payload }) => { state.notes = [...state.notes, payload]; state.saving = false })
...
export default contractModuleNoteSlice.reducer
