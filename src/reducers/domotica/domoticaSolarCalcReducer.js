...
const domoticaSolarCalcSlice = createSlice({
  name: 'domoticaSolarCalc',
  initialState: {
    data: null,
    error: {},
    fetching: false,
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(actions.successRequestUpdate, (state, { payload }) => {
        if (state.data) state.data = state.data.map((d) => (d.id === payload.id ? payload : d))
        state.fetching = false
      })
...
export default domoticaSolarCalcSlice.reducer
